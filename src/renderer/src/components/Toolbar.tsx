...
function Check({
  ...
  return (
    <label
      className={`flex items-center gap-2 text-sm select-none text-fg ${
...
export function Toolbar(props: ToolbarProps): JSX.Element {
  ...
  // While a recording is in progress (or paused) the audio source can't be
  // changed, so lock the mic picker and the capture toggles.
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 px-4 py-3 border-b border-border bg-bg">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="relative">
          <select
            className="field w-60 appearance-none pr-9 disabled:opacity-50 disabled:cursor-not-allowed"
            value={micDeviceId ?? ''}
      ...
      <Check checked={captureMic} onChange={setCaptureMic} label="Mic audio" disabled={locked} />
      <Check
      ...
      <div className="flex items-center gap-2.5 ml-auto">
        {recordingState === 'idle' && (
          <button className="btn" onClick={onLoadFile}>
            Load audio
          </button>
        )}
