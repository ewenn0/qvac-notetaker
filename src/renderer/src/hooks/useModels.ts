...
export interface UseModelsArgs {
  ...
  /** Reads the current LLM context window size (from summary settings). */
  getLlmCtxSize: () => number
}
...
export interface UseModels {
  ...
  loadLlm: (ctxSizeOverride?: number) => Promise<void>
...
export function useModels({ setError, getSttLanguage, getLlmCtxSize }: UseModelsArgs): UseModels {
  ...
  const getLlmCtxSizeRef = useRef(getLlmCtxSize)
  useEffect(() => {
    getLlmCtxSizeRef.current = getLlmCtxSize
  }, [getLlmCtxSize])
  ...
  const loadLlm = useCallback(async (ctxSizeOverride?: number) => {
    setError(null)
    try {
      setLlmLoaded(false)
      // Guard: onClick passes a MouseEvent as the first arg when loadLlm is
      // used directly as an onClick handler. Use typeof to reject non-numbers.
      const ctxSize = typeof ctxSizeOverride === 'number'
        ? ctxSizeOverride
        : getLlmCtxSizeRef.current()
